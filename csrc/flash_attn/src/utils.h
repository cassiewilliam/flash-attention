...
__forceinline__ __device__ auto convert_type(Tensor<Engine, Layout> const &tensor, To_type scale) {
    ...
    cutlass::multiplies<To_type> multiply_op;

    #pragma unroll
    for (int i = 0; i < numel; ++i) {
        frag[i] = multiply_op(frag[i], scale);
    }
...
template<bool A_in_regs=false, bool B_in_regs=false, typename Tensor0, typename Tensor1,
         typename Tensor2, typename Tensor3, typename Tensor4,
         typename TiledMma, typename TiledCopyA, typename TiledCopyB,
         typename ThrCopyA, typename ThrCopyB, typename Element>
__forceinline__ __device__ void gemm_q16_k8(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const& tCsA,
                            Tensor4 const& tCsB, TiledMma tiled_mma,
                            TiledCopyA smem_tiled_copy_A, TiledCopyB smem_tiled_copy_B,
                            ThrCopyA smem_thr_copy_A, ThrCopyB smem_thr_copy_B, Element k_scale) {
    CUTE_STATIC_ASSERT_V(size<1>(tCrA) == size<1>(acc));                     // MMA_M
    CUTE_STATIC_ASSERT_V(size<1>(tCrB) == size<2>(acc));                     // MMA_N
    CUTE_STATIC_ASSERT_V(size<2>(tCrA) == size<2>(tCrB));                     // MMA_K
    Tensor tCrA_copy_view = smem_thr_copy_A.retile_D(tCrA);
    ...
    for (int i = 0; i < size<2>(tCrA); ++i) {
        ...
        // Need more register
        cute::gemm(tiled_mma, tCrA(_, _, i), flash::convert_type(tCrB(_, _, i), k_scale), acc);
    }
}
...
template<typename Tensor0, typename Tensor1, typename Tensor2, typename Tensor3,
         typename TiledMma, typename TiledCopy, typename ThrCopy, typename Element>
__forceinline__ __device__ void gemm_p16_v8_rs(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const& tCsB,
                               TiledMma tiled_mma, TiledCopy smem_tiled_copy_B,
                               ThrCopy smem_thr_copy_B, Element v_scale) {
    CUTE_STATIC_ASSERT_V(size<1>(tCrA) == size<1>(acc));                     // MMA_M
    CUTE_STATIC_ASSERT_V(size<1>(tCrB) == size<2>(acc));                     // MMA_N
    CUTE_STATIC_ASSERT_V(size<2>(tCrA) == size<2>(tCrB));                     // MMA_K
    Tensor tCrB_copy_view = smem_thr_copy_B.retile_D(tCrB);
    ...
    for (int i = 0; i < size<2>(tCrA); ++i) {
        ...
        cute::gemm(tiled_mma, tCrA(_, _, i), flash::convert_type(tCrB(_, _, i), v_scale), acc);
    }
}
